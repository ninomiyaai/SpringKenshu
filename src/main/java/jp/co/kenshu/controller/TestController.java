...
public class TestController {
	...
	// 外部(DIコンテナ)からインスタンスを注入(生成) ※xmlにその設定を記述する必要がある
	@Autowired
	...
	public String testAll(Model model) {
		...
		model.addAttribute("message", "MyBatisの全権取得サンプルです");
	...
	public String testDto(Model model, @PathVariable int id) {
		...
		model.addAttribute("message", "MyBatisのサンプルです(where オブジェクト)");
		model.addAttribute("test", test);
		return "test";
	}
	...
	public String testIsert(@ModelAttribute TestForm form, Model model) {
		int count = testService.insertTest(form.getName());
		Logger.getLogger(TestController.class).log(Level.INFO, "挿入件数は" + count + "です");
